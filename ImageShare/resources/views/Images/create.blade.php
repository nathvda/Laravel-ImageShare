...
<form id="upload_form" method="post" action="/create" class="w-2/5 bottom-9 p-4 rounded-md bg-white justify-center shadow-md" enctype="multipart/form-data">
        ...
        <span class="text-red-500 text-sm">@if($errors->has('title'))
        {{$errors->first('title')}}
        @endif</span>
        <div class="flex justify-between items-center w-full mt-6">
        ...
        <span class="text-red-500 text-sm">@if($errors->has('url'))
        {{$errors->first('url')}}
        @endif</span>
        <div class="flex justify-between items-center w-full mt-6">
        ...
        <span class="text-red-500 text-sm">@if($errors->has('description'))
        ...
        <span class="text-red-500 text-sm">@if($errors->has('image'))
        {{$errors->first('image')}}
        @endif</span>

        <div class="flex justify-between items-center w-full"><button class="bg-slate-400 mt-5 p-2 rounded-md text-white from-slate-400 to-slate-700" type="submit">Add image</button></div>
    </form>
@endsection